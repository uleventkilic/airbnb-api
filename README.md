...
This is a RESTful API providing operations for an Airbnb-like platform, including property listing, guest booking, and host management. The API is built with Node.js and Express, with data managed in MongoDB.
...
### Admin Features
- Filter property listings based on country, city, and ratings.

### Host Features
- Add property listings with key details like price, country, and city.

### Guest Features
- Search for available listings using filters like dates and number of people.
- Make bookings for desired properties.
- Leave reviews for properties they’ve stayed in.
...
Use these credentials to log in and generate a JWT token for secured operations.
...
1. **API Layer**
   - Built with Express for efficient routing and middleware.
   - Endpoints for admin, host, and guest operations.

2. **Database Layer**
   - MongoDB for storing persistent data.
   - Includes collections for Users, Listings, Bookings, and Reviews.
...
Below is the ER diagram for the Airbnb API Project:

![Entity-Relationship Diagram](https://via.placeholder.com/600x400?text=Insert+ER+Diagram+Here)

---
# Airbnb-Like API Project
